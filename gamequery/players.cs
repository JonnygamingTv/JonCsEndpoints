...
public class Is_CsScript
{
    ...
    public static async Task Run(HttpContext context, string path)
    {
        ...
        if (string.IsNullOrEmpty(ip))
        {
            ...
            await context.Response.WriteAsync("Missing IP");
            return;
        }
        ...
        {
            context.Response.StatusCode = StatusCodes.Status400BadRequest;
            await context.Response.WriteAsync("Missing GAME");
            return;
        }
        int port;
        ...
        {
            context.Response.StatusCode = StatusCodes.Status400BadRequest;
            await context.Response.WriteAsync("Game not supported");
            return;
        }
        if(!int.TryParse(context.Request.Query["p"], out port))
